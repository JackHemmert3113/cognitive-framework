...
echo "🔁 Moving test documentation files into ./requirements/..."
...
for FILE in README.md TESTS.index.md TESTS.02_index.md TESTS.03_index.md; do
...
# Offer to delete the zip file
echo ""
read -p "🗑️  Do you want to delete the original ZIP file? (y/n): " confirm
if [[ $confirm == [yY] ]]; then
  ZIP_FILE="$(basename $0 .command)_requirements_unified.zip"
  if [ -f "$ZIP_FILE" ]; then
    rm "$ZIP_FILE"
...
echo "✅ All done! You can now access your docs under ./requirements/"
